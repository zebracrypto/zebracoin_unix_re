...
unsigned int nTargetSpacing = 2 * 30; // 30 seconds now 1min
...
const string strMessageMagic = "ZebraCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if(pindexBest->nHeight < 40101)
    {
        if (pindexBest->nHeight &1) //impairs
		{
			nSubsidy = 51*(1+0.5*dDiff/5000) * COIN;  //reward adjusted according to difficulty, based on empirical law for x11coins
			if(dDiff>=5000) nSubsidy = 81 * COIN;
		}
		else //pairs
		{
			nSubsidy = 101*(1+0.5*dDiff/5000) * COIN; //reward adjusted according to difficulty, based on empirical law for x11coins
			if(dDiff>=5000) nSubsidy = 151 * COIN;
		}
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "ZebraCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("2f9a618b61a159374d588ec6260794529193920880c06c15b5977f3d6578b3cf"));
        block.print();
